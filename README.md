...
ユニ・チャームの生理管理アプリ「ソフィBe」の口コミを収集し、新着だけをSlackチャンネルに通知するツール。
...
GitHub Actions (cron)
  → collector/main.py
      ...
      └ slack.py                … 新着だけ Incoming Webhook で通知
```
...
3. **リポジトリの Settings → Secrets and variables → Actions** に登録:
   ...
   - `X_BEARER_TOKEN`（任意。X開発者コンソールでアプリ作成しBearer Token取得。従量課金のクレジットをチャージ）
   - `ANTHROPIC_API_KEY`（任意。X投稿のノイズ除去用）
...
| 変数 | 意味 | デフォルト |
|---|---|---|
| `SLACK_WEBHOOK_URL` | 通知先Webhook。未設定なら標準出力に表示 | なし |
| `X_BEARER_TOKEN` | X API v2 のBearer Token。未設定ならXをスキップ | なし |
| `ANTHROPIC_API_KEY` | Claudeでの関連性判定。未設定ならキーワードのみ | なし |
