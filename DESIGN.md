...
- **フィルタサイドバーの原則**: sticky（デスクトップ）。**内容は常にビューポート内に収まるよう設計**し、すべての操作対象に手が届くこと。収まらない環境向けの保険として `max-height: calc(100vh - 64px); overflow-y: auto` を持つ。選択肢が多い群（バージョン）は**最新8件＋「すべて表示」トグル**で折りたたむ（選択中の値は折りたたみ中でも常に表示、`aria-expanded`）
